...
from ocf_datapipes.training.common import open_and_return_datapipes, create_t0_and_loc_datapipes
from ocf_datapipes.training.pvnet import (
    construct_loctime_pipelines, construct_sliced_data_pipeline, _get_datapipes_dict
)
from ocf_datapipes.utils.consts import ELEVATION_MEAN, ELEVATION_STD
from ocf_datapipes.batch import (
    stack_np_examples_into_batch, BatchKey, NumpyBatch, batch_to_tensor, copy_batch_to_device
)
...
def get_loctimes_datapipes(config_path):
    ...
    location_pipe = (
        IterableWrapper([[gsp_id_to_loc(gsp_id) for gsp_id in ALL_GSP_IDS]])
        .repeat(num_t0s)
    )
...
class ModelPipe:
    ...
    def predict_batch(self, batch: NumpyBatch):
        ...
        gsp_capacities = (
            ds_gsp.effective_capacity_mwp.sel(time_utc=t0, gsp_id=slice(1, None)).values
        )
        ...
        # If no summation model, make national predictions using simple sum
            ...
            da_abs_national = (
                da_abs_gsp.sum(dim="gsp_id").expand_dims(dim="gsp_id", axis=0).assign_coords(gsp_id=[0])
            )
        ...
        ds_abs_all = (
            ds_abs_all
            .expand_dims(dim="init_time_utc", axis=0)
            .assign_coords(init_time_utc=[t0])
        )
...
def get_datapipe(config_path: str):
    ...
    data_pipeline = (
        data_pipeline.batch(len(ALL_GSP_IDS))
