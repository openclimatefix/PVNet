...
if __name__=="__main__":
    ...
    sun_in = torch.rand((3, (history+forecast)//30+1))
    gsp_id = torch.rand((3, 1))
    # Shape: batch_size, seq_length, channel, height, width
    sat_data = torch.rand((3, history//5 +1 - 3, 11, 24, 24))
    # shape: batch_size, seq_len, n_chans, height, width
    nwp_data = torch.rand((3, (history+forecast)//60+1, 2, 24, 24))
    gsp = torch.rand((3, (history+forecast)//30+1))
